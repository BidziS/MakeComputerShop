...
namespace MakeComputerShop.Tests
{
    ...
    class SoundCardServiceTest
    {
        ...
        [Test]
        public void GetSoundCardByProducentId()
        {
            MapperConfig.Configuration();
        ...
        [Test]
        public void GetSoundCardByIdTest()
        {
            MapperConfig.Configuration();

            Mock<ISoundCardRepository> mock = new Mock<ISoundCardRepository>();

            mock.Setup(m => m.GetSoundCardById(1)).Returns(new SoundCardDb());

            var dto = Mapper.Map<SoundCardDb, SoundCardDto>(new SoundCardDb());

            ISoundCardService service = new SoundCardService(mock.Object);

            var actual = service.GetSoundCardById(1);
